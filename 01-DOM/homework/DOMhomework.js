...
// Tu código acá:
const creadoPor = document.querySelector('#createdBy');
creadoPor.innerHTML = creadoPor.innerHTML + "Juan Carlos";
...
function buildToDo(todo, index) {
  ...
  toDoText.innerHTTML = todo.description;
...
function buildToDos(toDos) {
  // Tu código acá:
  const nuevoArray = toDos.map(function buildToDo(todo, index) {
    // Tu código acá:
    const toDoShell = document.createElement('div');
    toDoShell.setAttribute('class', 'toDoShell');
    const toDoText = document.createElement('span');
...
function displayToDos() {
  ...
  resultado.forEach(element => toDoContainer += element);
}
...
function addToDo() {
  // Tu código acá:
  toDoItems.push(new ToDo(toDoInput));
  toDoInput = "";
  displayToDos();
}
